...
exports.handler = async function handler(event) {
  ...
  if (!Array.isArray(reviews)) {
    return jsonResponse(400, { error: 'reviews must be an array' });
  }
  ...
  let contentStr;
  try {
    contentStr = JSON.stringify(reviews, null, 2) + '\n';
    JSON.parse(contentStr);
  } catch (_err) {
    return jsonResponse(400, { error: 'reviews array failed JSON serialization' });
  }
  ...
  try {
    const putRes = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        message: 'Update reviews.json via reviews admin',
        content: contentB64,
        sha: remoteSha,
        branch: BRANCH,
      }),
